...
//    if let img = infoconvertFromUIImagePickerControllerInfoKey(UIImagePickerController.InfoKey.editedImage) as? UIImage {
...
extension ProfileVC {
  func tokenValidate() { // 토큰 인증 메소드
    ...
    NetworkActivityIndicatorManager.shared.isEnabled = true // 로딩 시작
  ...
  func refresh() { // 토큰 갱신 메소드
    ...
    refresh.responseJSON { res in
        NetworkActivityIndicatorManager.shared.isEnabled = false // 로딩 중지
      guard let jsonObject = res.value as? NSDictionary else {
        self.alert("잘못된 응답입니다.")
        return
      }
